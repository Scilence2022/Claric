...
function createCoordinationHandler({ store = new CoordinationStore(), token = process.env.COORDINATION_TOKEN || '', allowedOrigin = process.env.COORDINATION_ALLOWED_ORIGIN || '' } = {}) {
  ...
  return async function coordinationHandler(req, res) {
    ...
    const method = { '/coordination/healthz': 'GET', '/coordination/snapshot': 'GET', '/coordination/events': 'POST' }[url.pathname];
    if (!method) return json(res, 404, { error: 'Not found' });
    const params = [...url.searchParams.keys()];
    if (url.pathname === '/coordination/snapshot' ? params.length !== 1 || params[0] !== 'workspaceId' : params.length !== 0) return json(res, 400, { error: 'Invalid query parameters' });
    ...
    if (url.pathname === '/coordination/healthz') return json(res, 200, { status: 'ok', service: 'coordination', version: 1 });
    try {
      ...
      return json(res, 201, { event: store.append(await readBody(req)) });
...
if (require.main === module) {
  ...
  const server = createCoordinationServer({ tls: cert ? { cert: fs.readFileSync(cert), key: fs.readFileSync(key) } : undefined });
