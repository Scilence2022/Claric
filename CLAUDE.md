...
- Image generation has an independent provider/model route: OpenAI Images, Zhipu CogView, MiniMax Images, and Custom (OpenAI-compatible), with separate endpoint, API key, model, and size settings. OpenAI Images and Custom use `/images/generations`; CogView uses that endpoint and returns a hosted URL; MiniMax uses `/image_generation` with `aspect_ratio` and base64 output. The client normalizes all response forms to raw raster base64
